 ...
 * @LastEditTime: 2021-06-21 13:14:19
...
static void connect(rdma_context_t* context)
{
    ...
    size_t sz = sock_write(peer_sockfd, &local_qp_info, sizeof(local_qp_info));
    printf("|--sock_write[%zu/%zu]\n", sz, sizeof(local_qp_info));

    // recv
    qp_info_t remote_qp_info;
    sz = sock_read(peer_sockfd, &remote_qp_info, sizeof(remote_qp_info));
